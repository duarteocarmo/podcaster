...
class TTSDataset(Dataset):
    ...
    def _normalize_audio_inputs(
        ...
        for a in items:
            if isinstance(a, str):
                ...
                raise ValueError(
                    "For numpy waveform input, pass a tuple (audio, sr)."
                )
            else:
    ...
    def collate_fn(self, batch):
        ...
        for i, data in enumerate(batch):
            ...
            input_ids[i, 8 + text_ids_len - 3, 0] = (
                self.config.tts_eos_token_id
            )
            input_ids[
                i, 8 + text_ids_len - 2 : 8 + text_ids_len + codec_ids_len, 0
            ...
            codec_embedding_mask[i, 3 : 8 + text_ids_len + codec_ids_len] = (
                True
            )
